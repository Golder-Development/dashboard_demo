...
def donorsheadlinespage_body():
    ...
    donors_summary = (
        donors_df.groupby(["DonorName", "DonationType"])
        .agg(
            {
    ...
    donors_topline_summary = (
        donors_df.groupby(["DonorName"])
        .agg(
            {
                "Value": ["count", "sum", "mean", "median"],
                "RegulatedEntityName": "nunique",
    ...
    donors_topline_summary.columns = [
        "Donor Name",
        "No of Donations",
        "Total Value",
        "Average Donation",
    ...
    donors_topline_summary["Average Value per Regulated Entity"] = (
        donors_topline_summary["Total Value"]
        / donors_topline_summary["Regulated Entities"]
    )
    donors_topline_summary["Average Number Donations per Entity"] = (
        donors_topline_summary["No of Donations"]
        / donors_topline_summary["Regulated Entities"]
    )
    ...
    donors_topline_summary["Avg No. Donations Per Entity"] = donors_topline_summary[
        "Average Number Donations per Entity"
    ...
    with col1:
        ...
        st.write(
            f"* Between {min_date} and {max_date}, {donors} donors made "
    ...
    with col2:
        ...
        st.write(
            "* There are a few donors who make multiple donations, and a "
            " few who make donations to multiple regulated entities."
            " These donations are generally larger in value."
        )
        st.write(
            "* From the charts below, we can see that on average, donors"
        ...
        st.write(
            "* The top 3 most generous donors overall where all Trade"
            " Unions, "
            " as were all 5 of the most active donors by number of"
            " donations. "
        )
        st.write(
            "* The top 5 most generous on average donors were all"
            " individuals,"
        ...
        st.write(
            "* When we look at the Average value of donations per"
            " regulated entity"
        ...
        st.write(
            "* Unsurprisingly, the more Entities donated to more" " donations made."
        )
    st.write("---")
    ...
    with left:
        ...
        vis.plot_custom_bar_chart(
            ...
            group_column=None,
            agg_func="count",
            title="Count of Donors vs" " No of Regulated Entities",
    ...
    st.write("---")
    donors_summary2 = donors_topline_summary.sort_values(
        "Regulated Entities", ascending=False
    )
    donors_summary2 = donors_summary2[
        [
            "Donor Name",
            "Regulated Entities",
    ...
    st.write("---")
    donors_summary2 = donors_topline_summary.sort_values(
        "Average Donation", ascending=False
    )
    donors_summary2 = donors_summary2[
        [
            "Donor Name",
    ...
    donors_summary2 = donors_topline_summary.sort_values(
        "No of Donations", ascending=False
    )
    donors_summary2 = donors_summary2[
        [
            "Donor Name",
    ...
    st.write("---")
    st.write(
        "For more detailed analysis of individual donors, please"
        " visit the [Donors Per Donor Page](donors_per_donor_page.py)."
    )
    st.write("---")
