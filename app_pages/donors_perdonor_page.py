...
def donorspage_body():
    ...
    date_filter = (donors_df["ReceivedDate"] >= start_date) & (
        donors_df["ReceivedDate"] <= end_date
    )
    ...
    selected_entity_name = st.selectbox(
        "Filter by Donor", ["All"] + sorted(map(str, entity_mapping2.keys()))
    )
    ...
    if filters:
        donors_r_d_df = donors_d_df[donors_d_df["DonorId"] == selected_entity_id]
    else:
    ...
    donors_summary = (
        donors_d_df.groupby(["DonorName", "DonationType"])
        .agg(
            {
                "Value": ["count", "sum", "mean", "median"],
                "RegulatedEntityName": "nunique",
            }
        )
        .reset_index()
    )
    donors_summary.columns = [
        "Donor Name",
        "Donation Type",
        "No of Donations",
        "Total Value",
    ...
    donors_summary["Total Value"] = pd.to_numeric(
        donors_summary["Total Value"], errors="coerce"
    )
    donors_summary["Regulated Entities"] = pd.to_numeric(
        donors_summary["Regulated Entities"], errors="coerce"
    )
    ...
    donors_entity_summary = (
        donors_d_df.groupby(["DonorName"])
        .agg(
            {
                "Value": ["count", "sum", "mean", "median"],
                "RegulatedEntityName": "nunique",
    ...
    for col in [
        ...
        donors_entity_summary[col] = pd.to_numeric(
            donors_entity_summary[col], errors="coerce"
        )
    donors_entity_summary.columns = [
        "Regulated Entity",
        "No of Donations",
    ...
    donors_entity_summary["Avg Value per Entity"] = donors_entity_summary.apply(
        lambda row: (
            row["Total Value"] / row["Regulated Entities"]
            if row["Regulated Entities"] > 0
            else 0
        ),
        axis=1,
    )
    donors_entity_summary["Average Number Donations per Entity"] = (
        donors_entity_summary.apply(
            lambda row: (
                row["No of Donations"] / row["Regulated Entities"]
                if row["Regulated Entities"] > 0
                else 0
            ),
            axis=1,
    ...
    donors_entity_summary["Regulated Entities_f"] = donors_entity_summary[
        "Average Number Donations per Entity"
    ...
    for col in [
        ...
        donors_entity_summary[col] = donors_entity_summary[col].apply(
            lambda x: f"£{ppcalc.format_number(x)}"
        )
    donors_entity_summary["Avg No. Donations Per Entity"] = donors_entity_summary[
        "Average Number Donations per Entity"
    ...
    st.write("---")
    st.write(
        f"## Analysis of Political Donations for {selected_entity_name}"
        f"## between {min_date_f} and {max_date_f}"
    )
    if selected_entity_name == "All":
    ...
    with col1:
        st.write(
            "* For this analysis, any donations made by Political Parties"
    ...
    with col2:
        st.write(
            " * As can be easily seen from the graph, there is a vast"
    ...
    with mid:
        ...
        st.write(
            "<div style='text-align: center;'><b>Size of circles "
