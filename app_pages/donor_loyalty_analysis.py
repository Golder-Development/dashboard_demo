...
from Visualisations import plot_bar_chart
...
def mod_donor_loyalty():
    ...
    # Exclude Unidentified Donors and The Electoral Commission
    donor_df = donor_df[
        donor_df['DonorName'] != 'Unidentified Donor'
    ].copy()
    donor_df = donor_df[
        donor_df['DonorName'] != 'The Electoral Commission'
    ].copy()
...
def display_donor_distribution(donor_df, parties_list):
    ...
    Note: Aggregations in this function are context-specific (filtering,
    multi-column groupby, nunique operations) and are intentionally kept
    as inline code rather than using calculate_agg_by_variable() which is
    designed for simpler single-column groupby operations on numeric columns.
    """
    st.write("""
    ...
    with col1:
        ...
        # Convert Series to DataFrame for standardized visualization
        party_dist_df = pd.DataFrame({
            'Parties_Per_Donor': party_dist.index.astype(str),
            'Count': party_dist.values
        })
        
        plot_bar_chart.plot_custom_bar_chart(
            graph_df=party_dist_df,
            XValues='Parties_Per_Donor',
            YValues='Count',
            title='Distribution of Parties per Donor',
            XLabel='Number of Parties',
            YLabel='Number of Donors',
            orientation='v',
            agg_func='sum',
            use_custom_colors=False,
            widget_key='donor_party_distribution'
        )
...
def display_government_period_analysis(donor_df, parties_list):
    ...
    Note: Aggregations in this function filter data by time period before
    aggregating, making them unsuitable for calculate_agg_by_variable() 
    which requires pre-processing. Kept as inline code for clarity.
    """
    st.write("""
    ...
    with col1:
        ...
        if pivot_data:
            ...
            pivot_display = normalize_string_columns_for_streamlit(
                pivot_display.fillna(0).astype(int)
            )
    ...
    with col2:
        ...
        if value_data:
            ...
            value_df = normalize_string_columns_for_streamlit(value_df)
...
def display_loyalty_metrics(donor_df, parties_list):
    ...
    Note: Aggregations in this function use complex multi-column groupby 
    operations and custom loyalty calculations that exceed the scope of 
    calculate_agg_by_variable(). Inline code is intentional for domain logic.
    """
    st.write("""
    ...
    if loyalty_list:
        ...
        with col1:
            ...
            plot_bar_chart.plot_custom_bar_chart(
                ...
                XLabel='Loyalty Score Range',
                YLabel='Number of Donors',
                orientation='v',
                agg_func='sum',
                use_custom_colors=False,
                widget_key='loyalty_score_distribution'
            )
        ...
        with col2:
            ...
            top_loyal = normalize_string_columns_for_streamlit(top_loyal)
        ...
        if len(power_followers) > 0:
            ...
            power_followers_display = normalize_string_columns_for_streamlit(
                power_followers.head(10)
            )
            st.dataframe(power_followers_display.style.format(
                {'Loyalty_Score': '{:.2%}'}
            ))
