...
def dedupe_entity_file(
    ...
    if os.path.exists(map_filename):
        ...
        loaddata_dd_df["ParentEntityId"] = loaddata_dd_df[cleanedentityid].replace(
            "", pd.NA
        )
        loaddata_dd_df["ParentEntityName"] = loaddata_dd_df[cleanedentityname].replace(
            "", pd.NA
        )
        loaddata_dd_df["ParentEntityId"] = loaddata_dd_df["ParentEntityId"].fillna(
            loaddata_dd_df[entityid]
        )
        loaddata_dd_df["ParentEntityName"] = loaddata_dd_df["ParentEntityName"].fillna(
...
def dedupe_entity_fuzzy(deupedf, entity, threshold=85, output_csv=False):
    ...
    for main_id, duplicate_ids in potential_duplicates.items():
        ...
        if entityid in loaddata_dd_df.columns:
            matching_names = (
                loaddata_dd_df[loaddata_dd_df[entityid].isin(all_ids)][entityname]
                if not loaddata_dd_df[loaddata_dd_df[entityid].isin(all_ids)].empty
                else pd.Series(dtype=str)  # Avoid deprecated empty Series
            )
        else:
            matching_names = pd.Series(dtype=str)
    ...
    if isinstance(searchid, int):
        searchid = str(searchid)  # Ensure entityid is a string

    if entityid in loaddata_dd_df.columns:
        loaddata_dd_df[entityid] = loaddata_dd_df[entityid].replace("", pd.NA)
        ...
        print(f"Warning: Column '{entityid}' not found in loaddata_dd_df")
