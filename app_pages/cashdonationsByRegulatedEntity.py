def cashdonations_body():
    ...
    cleaned_df = st.session_state.get("data_clean", None)  # Load dataset from session state
    ...
    if filters:
        cleaned_r_d_df= cleaned_d_df[cleaned_d_df["RegulatedEntityId"] == filters["RegulatedEntityId"]]
    else:
    ...
    unique_regulated_entities_c_d = ppcalc.get_regentity_ct(cleaned_c_d_df, filters)
    ...
    unique_donations_c = ppcalc.get_donations_ct(cleaned_df, {"DonationType": "Cash"})
    perc_cash_donations = (unique_donations_c / unique_donations_pop) * 100 if unique_donations_pop > 0 else 0
    perc_cash_donations_d = (unique_donations_c_d / unique_donations_d) * 100 if unique_donations_d > 0 else 0
    ...
    st.write(f"* During the period between {min_date_df} and {max_date_df}, there were "
             f"{unique_donations_c_d} cash donations made to {unique_regulated_entities_c_d}.")
    st.write(f"* These had a mean value of £{ppcalc.format_number(mean_value_donations_c_d)} "
             f"and were made by {ppcalc.format_number(unique_donors_c_d)} unique donors.")
    st.write(f"* Cash donations represented {perc_cash_donations_d:.2f}% of all donations "
             f"during the period selected, and {perc_cash_donations:.2f}% of all donations "
             f"made to political parties between {min_date} and {max_date},.")
    st.write(f"* All these had a value of £{ppcalc.format_number(total_value_donations_c_d)}")
    st.write("---")
