...
def check_password(username, password):
    ...
    if username == credentials["admin_username"]:
        return bcrypt.checkpw(password.encode(),
                              credentials["admin_password_hash"].encode())
    return False


# Function to load text for a specific page
def load_page_text(target_label):
    all_texts = load_all_text()
    return all_texts.get(target_label, {})


# Function to toggle soft delete
def toggle_soft_delete(target_label, text_key, delete_status):
    if "TEXT_FILE" not in st.session_state:
        Refresh_Text_Session_State()
    else:
        TEXT_FILE = st.session_state.get("TEXT_FILE")
    all_texts = load_all_text()
    if target_label in all_texts and text_key in all_texts[target_label]:
        all_texts[target_label][text_key]["is_deleted"] = delete_status
        with open(TEXT_FILE, "w", encoding="utf-8") as f:
            json.dump(all_texts, f, indent=4)


# Function to permanently delete a text element
def permanent_delete(target_label, text_key):
    if "TEXT_FILE" not in st.session_state:
        Refresh_Text_Session_State()
    else:
        TEXT_FILE = st.session_state.get("TEXT_FILE")
    all_texts = load_all_text()
    if target_label in all_texts and text_key in all_texts[target_label]:
        del all_texts[target_label][text_key]
        if not all_texts[target_label]:
            del all_texts[target_label]
        with open(TEXT_FILE, "w", encoding="utf-8") as f:
            json.dump(all_texts, f, indent=4)
...
# Function to load text for a specific page
def load_page_text(text_label):
    all_texts = load_all_text()
    return all_texts.get(text_label, {})  # Return dictionary of text elements


# Function to save text for a specific page and element
def save_text(target_label, text_key, new_text):
    if "TEXT_FILE" not in st.session_state:
        Refresh_Text_Session_State()
    else:
        TEXT_FILE = st.session_state.get("TEXT_FILE")
    all_texts = load_all_text()
    if target_label not in all_texts:
        all_texts[target_label] = {}
    all_texts[target_label][text_key] = new_text
    with open(TEXT_FILE, "w", encoding="utf-8") as f:
