...
def donorsheadlinespage_body():
    ...
    """
    donors_df2 = st.session_state.get("data_clean", None)
    donors = ppcalc.get_donors_ct(donors_df2)
    donations = ppcalc.get_donations_ct(donors_df2)
    totaldonations = ppcalc.get_value_total(donors_df2)
    min_date = ppcalc.get_mindate(donors_df2).date()
    max_date = ppcalc.get_maxdate(donors_df2).date()
    ...
    with col1:
        ...
        # create x,y graph showing average donor donation vs total number of
    ...
    with col2:
        ...
        if donors_df2 is not None:
            # Display the first 5 rows of the dataset
            st.write(donors_df2.describe())
        else:
